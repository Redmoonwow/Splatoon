...
internal unsafe class P5_Delta_Hello_Guide :SplatoonScript
{
    ...
    public override void OnGainBuffEffect(uint sourceId, Status Status)
    {
        ...
        if(Status.StatusId == BuffID.BashDebuff)
        {
        ...
        else if(Status.StatusId == BuffID.SampledBuffLeft ||
                 Status.StatusId == BuffID.SampledBuffRight)
        {
            var player = _partyData.FirstOrDefault(x => x.player.EntityId == sourceId);
            if(player != null)
            {
                player.isSampled = true;
                player.SampledLeftRight = Status.StatusId == BuffID.SampledBuffLeft ? "Left" : "Right";
            }
        }
    }
