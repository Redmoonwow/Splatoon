...
internal unsafe class PrepullTest :SplatoonScript, IDisposable
{
    ...
    public override void OnSetup()
    {
        try
        {
            CountDownDetector.Init();
        }
        catch (Exception e)
        {
            PluginLog.Error($"Error in PrepullTest: {e}");
        }
    }

    public override void OnSettingsDraw()
    {
        if (!CountDownDetector.isEnabled) return;
        ImGui.Text($"countDownActive: {CountDownDetector.IsActiveCountDown()}");
        ImGui.Text($"countDownPointerValue: {CountDownDetector.GetCountDownRemainingTime()}");
    }
